...
void PrintHospitalData (vector<Hospital>& hospitals)
{
	...
	for (int i = 0; i < 4; i++) {
		cout<< hospitals[i].Hospital_name
			<<  "	     	 "
			<<  "     "
	...
	string doctor_Name[], int prices[])
{
	...
	// Initialize the data
	for (int i = 0; i < 4; i++) {
		p.Patient_name = patient_Name[i];
		p.Patient_ID = patient_Id[i];
	...
	PrintHospitalBycity("Hyderabad",hospitals);
...
int main()
{
	...
	// the user data
	string patient_Name[] = { "Aman", "Raju", "Sai", "Tanay" };
	int patient_Id[] = { 235, 327, 451, 157 };
	string patient_Contact[]
		= { "93453XXXX7", "73457XXXX2", "85746XXXX9", "9345XXXX0" };
	int bookingCost[] = { 1200, 1600, 1300, 800 };

	string hospital_Name[] = { "PRIME", "RISE", "ANAND", "AMK" };
	string locations[] = { "Hyderabad", "Bangalore", "Mumbai ", "Chennai" };
	int beds[] = { 4, 5, 6, 9 };
	float ratings[] = { 6.2, 4.6, 4.4, 5.8 };
	string hospital_Contact[]
		= { "657534XXX7", "998766XXX2", "724565XXX9", "843456XXX4" };
	string doctor_Name[] = { "Rohit", "Ram", "Valli", "Ramya" };
